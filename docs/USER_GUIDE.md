...
*   **Merge Roll to TIFF…**: writes each loaded triplet as one untagged 16-bit linear TIFF, `<red name>_RGB.tif`, next to its red exposure, and swaps it into the film strip. The frame keeps its edit, triage mark, scene and roll forks, and a `.negpy` sidecar moves with it. The export demosaic is fixed in the file; flat-field and rotation stay in the edit. With **Move each merged frame's exposures to the Trash** (on by default), the three exposures and the red one's sidecar go to the Trash after the file is read back and matched; a frame that fails keeps them. Triplets inside a stitch are skipped.
