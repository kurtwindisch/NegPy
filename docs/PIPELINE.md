...
*   **Trichrome triplets**: all three narrowband exposures decode and merge through `merge_rgb_triplet()` into one TIFF. The red exposure is primary (and supplies WB and device metadata); green and blue paths come from the frame's `RgbScanConfig`. No sensor correction.
    ...
    **Merge Roll to TIFF** (`services/export/triplet_merge.py`) writes the render decode (`ImageProcessor._decode_oriented_f32` with flat-field off), not this decode, so the file holds the exact uint16 source the pipeline inverts for the triplet and the untagged TIFF loads back to the same buffer. The frame's edit is copied to the new content hash with `rgbscan` cleared and the sensor matrix dropped (`merged_edit`): a triplet skips the unmix and a single file would apply it. Discovery treats a TIFF whose description names an RGB triplet as one frame and never groups it.
