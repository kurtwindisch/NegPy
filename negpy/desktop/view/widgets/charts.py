...
class PhotometricCurveWidget(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        # Which layer(s) to draw: 'all' | 'curve' | 'print_hist' | 'density_hist'.
        self._overlay_mode: str = "all"
    ...
    def set_overlay_mode(self, mode: str) -> None:
        """All layers together, or isolate one: 'all' | 'curve' | 'print_hist' | 'density_hist'."""
    ...
    def paintEvent(self, event) -> None:
        ...
        # Histograms under everything else so the curve/zone tints stay readable. Peek
        # Negative already forces _show_print False, leaving density as the only content,
        # so it stays visible then regardless of the isolate choice below.
        if self._overlay_mode in ("all", "density_hist") or not self._show_print:
            self._draw_density_histogram(painter, w, h)

        if self._show_print:
            show_curve = self._overlay_mode in ("all", "curve")
            show_print_hist = self._overlay_mode in ("all", "print_hist")
